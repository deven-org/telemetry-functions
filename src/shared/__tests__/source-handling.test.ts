...
describe("source-handling", () => {
  ...
  describe("validateRequest", () => {
    it("returns status code 400 when signature header is not set", () => {
      const returnValue = validateRequest({}, "", "");

      expect(returnValue?.statusCode).toBe(400);
      expect(returnValue?.message).toBe(LogErrors.badRequestError);
    });
    it("returns status code 401 when signature can not be verified", () => {
      ...
      expect(returnValue?.message).toBe(LogErrors.forbiddenRequestError);
    });
